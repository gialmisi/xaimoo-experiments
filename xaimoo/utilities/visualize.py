...
def plot_rule_explanations(df: pd.DataFrame, objective_names: list[str], rule_check_column: str, true_check_colum: str, truth_value: int, title: str = ""):
    ...
    false_neg_group = df[(df[true_check_colum] != truth_value) & (df[rule_check_column] == True)]
    false_pos_group = df[(df[true_check_colum] == truth_value) & (df[rule_check_column] == False)]
    ...
    for i, pair in enumerate(objective_pairs):
        ...
        for simplex in hull.simplices:
            axs[i].plot(points[simplex, 0], points[simplex, 1], "--", c="grey")
