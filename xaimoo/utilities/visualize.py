...
def plot_rule_explanations(df: pd.DataFrame, objective_names: list[str], rule_check_column: str, true_check_colum: str, truth_value: int, title: str = ""):
    ...
    print(f"True positives  = {sum((df[true_check_colum] == truth_value) & (df[rule_check_column] == True))} / {sum(df[true_check_colum] == truth_value)}")
    print(f"True negatives  = {sum((df[true_check_colum] != truth_value) & (df[rule_check_column] == False))} / {sum(df[true_check_colum] != truth_value)}")
    print(f"False positives = {sum((df[true_check_colum] != truth_value) & (df[rule_check_column] == True))} / {sum(df[rule_check_column] == True)}")
    print(f"False negatives = {sum((df[true_check_colum] == truth_value) & (df[rule_check_column] == False))} / {sum(df[rule_check_column] == False)}")
    ...
    for i, pair in enumerate(objective_pairs):
        ...
        for simplex in rule_hull.simplices:
            axs[i].plot(rule_points[simplex, 0], rule_points[simplex, 1], "--", c="green")
